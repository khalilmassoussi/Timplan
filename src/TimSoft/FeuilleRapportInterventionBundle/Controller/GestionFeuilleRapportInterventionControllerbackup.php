...
class GestionFeuilleRapportInterventionController extends Controller
{
    ...
    public function remplirFeuillePresenceAction(Request $request)
    {
        ...
        $em->flush();
        //envoi des e-mails
        $message = (new \Swift_Message('Informations concernant la Feuille de présence'))
            ->setFrom(['kaoutherchtimsoft@gmail.com' => "Administrateur TimSoft"])
            ->setBody(
                $this->renderView(
                    '@TimSoftFeuilleRapportIntervention/Emails/emailFeuille.txt.twig', array('Feuille' => $FeuilleDePresence)
                ), 'text/html');
        $failedRecipients = [];
        $numSent = 0;
        //generation PDF
        ...
        $em->flush();
        return new JsonResponse($FeuilleDePresence);

    }
//        $form1 = $this->createForm('TimSoft\GeneralBundle\Form\RapportInterventionType', $RapportIntervention);
//        $form = $this->createForm('TimSoft\GeneralBundle\Form\FeuilleDePresenceType', $FeuilleDePresence);
//        if ($request->isMethod('POST')) {
//            $form->handleRequest($request);
//            $form1->handleRequest($request);
//        }
//        if ($form->isValid()) {
//            $FeuilleDePresence = $form->getData();
//            $FeuilleDePresence->setIntervenant($this->getUser());
//            $LieuIntervention = $form->get('modeIntervention')->getData();
//            if ($LieuIntervention === "Sur site") {
//                $FeuilleDePresence->setLieuIntervention($form->get('client')->getData()->getAdresseClient());
//            }
//            if ($LieuIntervention === "À diastance") {
//                $FeuilleDePresence->setLieuIntervention("Tunis");
//            }
//            if ($LieuIntervention === "Mode projet") {
//                $FeuilleDePresence->setLieuIntervention("Tunis");
//            }
//            $em = $this->getDoctrine()->getManager(); // initialise la connexion à la BD
//            $em->persist($FeuilleDePresence); //L'enregistrement se fait uniquement dans Doctrine
...
//            // Render the HTML as PDF
//            $d->render();
//            $fileName = 'Feuille de présence du ' . $FeuilleDePresence->getDateIntervention()->format('d-m-Y') . ' par ' . $FeuilleDePresence->getIntervenant() . ' ' . $FeuilleDePresence->getClient();
//            $fileName = str_replace('/', '', $fileName);
//            $directory = $this->get('kernel')->getRootDir() . '/../web/PDF/' . $fileName . '.pdf';
//            $pdf_string = $d->output();
//            file_put_contents($directory, $pdf_string);
...
//            $notification
//                    ->setTitle('Nouvelle Feuille de présence')
//                    ->setDescription('Client: ' . $FeuilleDePresence->getClient() . '- Intervenant: ' . $FeuilleDePresence->getIntervenant())
//                    ->setResponsable($this->getUser())
//                    ->setRoute('ConsulterFeuillePresence')
//                    ->setParameters(array('id' => $FeuilleDePresence->getId()));
//            $em->persist($notification);
//            $em->flush();
////            $pusher = $this->get('mrad.pusher.notificaitons'); // 3aiet lel service
////            $pusher->trigger($notification);
//
//            foreach ($FeuilleDePresence->getParticipants() as $Participant) {
//                //Envoi d'un Mail au participant
//                $message->setTo([$Participant->getAdresseMailPArticipant() => $Participant->getNomParticipant() . ' ' . $Participant->getPrenomParticipant()]);
...
//                $pusher->trigger($notificationUser);
//            }
//            $this->addFlash('OK-Ajout', 'La feuille ' . $FeuilleDePresence . ' est ajoutée avec succès !');
//            return $this->redirectToRoute('ConsulterFeuillePresence', array('id' => $FeuilleDePresence->getId()));
//        }
//
//        if ($form1->isValid()) {
//            $RapportIntervention = $form1->getData();
//            $em = $this->getDoctrine()->getManager(); // initialise la connexion à la BD
//            $em->persist($RapportIntervention); //L'enregistrement se fait uniquement dans Doctrine
//            $em->flush(); // pour sauvegarder les données dans la BD
//
//            $message = (new \Swift_Message('Rapport d\'intervention'))
//                    ->setFrom(['kaoutherchtimsoft@gmail.com' => "Administrateur TimSoft"])
//                    ->setBody(
//                    $this->renderView(
//                            '@TimSoftFeuilleRapportIntervention/Emails/emailRapport.txt.twig', array('Rapport' => $RapportIntervention)
//                    ), 'text/html');
//            $failedRecipients = [];
//            $numSent = 0;
//            foreach ($RapportIntervention->getFichiersJoin() as $Fichier) {
...
//                }
//            }
//            $view = $this->renderView('@TimSoftFeuilleRapportIntervention/Telechargement/TelechargerUnRapport.html.twig', array('Rapport' => $RapportIntervention));
//            // On crée une  instance pour définir les options de notre fichier pdf
//            $options = new Options();
//            // Pour simplifier l'affichage des images, on autorise dompdf à utiliser
//            // des  url pour les nom de  fichier
...
//            // Render the HTML as PDF
//            $d->render();
//            $fileName = 'Rapport de l\'intervention du ' . $RapportIntervention->getFeuilleDePresence()->getDateIntervention()->format('d-m-Y') . ' par ' . $RapportIntervention->getFeuilleDePresence()->getIntervenant() . ' ' . $RapportIntervention->getFeuilleDePresence()->getClient();
//            $fileName = str_replace('/', '', $fileName);
//            $directory = $this->get('kernel')->getRootDir() . '/../web/PDF/' . $fileName . '.pdf';
//            $pdf_string = $d->output();
//            file_put_contents($directory, $pdf_string);
...
//                    ->setDescription('Client: ' . $RapportIntervention->getFeuilleDePresence()->getClient() . '- Intervenant: ' . $RapportIntervention->getFeuilleDePresence()->getIntervenant())
//                    ->setResponsable($this->getUser())
//                    ->setRoute('ConsulterRapportIntervention')
//                    ->setParameters(array('id' => $RapportIntervention->getId()));
//            $em->persist($notification);
//            $em->flush();
//
//            foreach ($RapportIntervention->getFeuilleDePresence()->getParticipants() as $Participant) {
//                $message->setTo([$Participant->getAdresseMailPArticipant() => $Participant->getNomParticipant() . ' ' . $Participant->getPrenomParticipant()]);
//                $message->attach(\Swift_Attachment::fromPath($directory));
//                $numSent += $this->get('mailer')->send($message, $failedRecipients);
//
...
//                $pusher->trigger($notificationUser);
//               }
//                return $this->redirectToRoute('ConsulterRapportIntervention', array('id' => $RapportIntervention->getId()));
//
//        }
//        return $this->render('@TimSoftFeuilleRapportIntervention/GestionFeuilleRapportIntervention/RemplirFeuilleRapport.html.twig', array(
...
//        ));
    ...
    public function consulterFeuillesRapportInterventionAction()
    {
        ...
        $em = $this->getDoctrine()->getManager(); // initialise la connexion à la BD
        $Feuilles = $em->getRepository('TimSoftGeneralBundle:FeuilleDePresence')->findBy(
            array(),
            array('id' => 'DESC')
        );
...
//        }
    ...
    /**
     * @Route("/ConsulterFeuillePresence/{id}", name="ConsulterFeuillePresence")
     */
...
//        }
    ...
    /**
     * @Route("/ConsulterRapportIntervention/{id}", name="ConsulterRapportIntervention")
     */
...
//        }
    ...
    /**
     * @Route("/SupprimerUneFeuille/{id}", name="SupprimerUneFeuille")
     */
...
//                $this->addFlash('OK-Suppression', 'La feuille de présence est supprimée avec succès  !');
    ...
    /**
     * @Route("/SupprimerFeuille/{id}", name="SupprimerFeuille")
     */
    ...
    /**
     * @Route("/SupprimerUnRapport/{id}", name="SupprimerUnRapport")
     */
...
//                        die();
    ...
    /**
     * @Route("/ModifierRapportIntervention/{id}", name="ModifierRapportIntervention")
     */
                ...
                if ($form->isValid()) {
                    ...
                    if ($Rapport->getDone() == 0) {
                        ...
                        if ($Feuille->getStatutValidation() == 1) {
                            ...
                            $failedRecipients = [];
                            $numSent = 0;
                            $messageRedaction->setCc([$Feuille->getIntervenant()->getEmail() => $Feuille->getIntervenant()->getNomUtilisateur() . ' ' . $Feuille->getIntervenant()->getPrenomUtilisateur()]);
                            ...
                            foreach ($gestionnaires as $gestionnaire) {
                                $messageRedaction->addTo($gestionnaire->getEmail(), $gestionnaire->getNomUtilisateur() . ' ' . $gestionnaire->getPrenomUtilisateur());
                            }
...
//                                $bu = $this->getDoctrine()->getRepository('TimSoftBuBundle:Bu')->findOneBy(array('libelle' => $Feuille->getIntervention()->getLc()->getBu()));
//                                foreach ($buManager->getBus() as $bus) {
//                                    if ($bu == $bus) {
//                                        $messageRedaction->addTo($buManager->getEmail(), $buManager->getNomUtilisateur() . ' ' . $buManager->getPrenomUtilisateur());
//                                    }
//                                }
//                            }
...
//            }
    ...
    /**
     * @Route("/TelechargementFeuillePresence/{id}", name="TelechargerFeuillePresence")
     */
    public
...
//            return new Response('', 200, array('Content-Type' => 'application/pdf'));
    ...
    /**
     * @Route("/TelechargementRapportIntervention/{id}", name="TelechargerRapportIntervention")
     */
    public
