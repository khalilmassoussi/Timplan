    ...
    public function onKernelRequest(GetResponseEvent $event)
    {
        if (Kernel::MAJOR_VERSION == 2 && Kernel::MINOR_VERSION < 6) {
            ...
            if (null !== $securityContext && null !== $securityContext->getToken() && $securityContext->isGranted('IS_AUTHENTICATED_REMEMBERED')) {
                ...
                $loggable->setUsername($securityContext->getToken()->getNomUtilisateur() . ' ' . $securityContext->getToken()->getPrenomUtilisateur());
            ...
            if (null !== $tokenStorage && $authorizationChecker->isGranted('IS_AUTHENTICATED_REMEMBERED')) {
                # for loggable behavior
                $loggable = $this->container->get('gedmo.listener.loggable');
                $loggable->setUsername($tokenStorage->getUser()->getPrenomUtilisateur() . ' ' . $tokenStorage->getUser()->getNomUtilisateur());
